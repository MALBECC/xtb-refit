...
class XtbRunner:
    ...
    def run(self):
        ...
        try:
            ...
            calc.set_verbosity(0)
            calc.set_accuracy(0.0001)
            # Optional SCF-robustness overrides (opt-in via env; unset => original
            # behavior of 150 iterations at the xtb default 300 K electronic temp).
            calc.set_max_iterations(int(os.environ.get('XTB_MAXITER', '150')))
            _etemp = os.environ.get('XTB_ETEMP')
            if _etemp:
                calc.set_electronic_temperature(float(_etemp))
            res = calc.singlepoint()
        except Exception as e:
